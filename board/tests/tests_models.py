...
class NewspaperModelTests(TestCase):
    ...
    def setUp(self):
        self.user = User.objects.create_user(username='redactor1', password='testpassword')
        self.topic = Topic.objects.create(name='Technology')

    def test_create_newspaper(self):
        newspaper = Newspaper.objects.create(title='Tech News', content='Latest updates in technology.')
        ...
        self.assertEqual(str(newspaper), f"{newspaper.title} {newspaper.publisher_date}")

class CommentModelTests(TestCase):

    def setUp(self):
        self.user = User.objects.create_user(username='redactor1', password='testpassword')
        self.newspaper = Newspaper.objects.create(title='Tech News', content='Latest updates in technology.')

    def test_create_comment(self):
        comment = Comment.objects.create(content='Great article!', newspaper=self.newspaper, redactor=self.user)
        ...
        self.assertEqual(str(comment), f"Comment by {self.user.username} on {self.newspaper.title}")
